...
phase accumulators for everyone note (12 * 3 for each keyboard). It added the current step size corresponding to each note (if it was being pressed) to each accumulator which overflowed at a given rate, (based on the step size, i.e the note frequency) and thus produced a sawtooth waveform. 
...
This ISR was activated when the waveform was set to triangle wave. It operation is very similar to the sawtooth ISR. There are two main differences.
...
the transition from 0 t0 255 had to happen twice as fast.
